...
- **Empty-die handling is operator-specific.**
  ...
  - `+`, `-`: a *single* empty operand coerces to scalar `0` (so `d{} + 5 = 5`); *both* operands empty propagates `H({})`. The both-empty case is operationally important -- it shows up when `result: [f] + [f]` hits a recursion cap from both sides; treating it as `0 + 0 = 0` would leak a spurious `0` into the parent distribution (see Section 3). AnyDice exhibits LHS-asymmetric bugs under these ops too (`d{} + {1,2,3}` returns the seq per-element rather than sum-coercing; `3d6 + d{}` preserves the pool's positional structure rather than collapsing -- as if `d{}` were a no-op identity on the non-empty side instead of a coerced scalar `0`). Our uniform scalar-0-coercion produces the principled answer; the divergence is deliberate. See annotations `0x42aa4` / `0x1857f`.
  - `*`, `/`, `^`: any empty operand propagates `H({})`. Empty is the multiplicative annihilator, not a coerced `0`.
  - `&`: any empty operand propagates `H({})`.
  - `|`: same scalar-handling shape as `+` / `-` (single empty operand coerces to `0`; both empty propagates `H({})`), but `|` ALSO inherits the LHS-asymmetric AnyDice bugs documented under corpus `0x42aa4`: with `d{}` on the left and a seq on the right, AnyDice returns the seq's elements per-element (`d{} | {1,2,3}` -> pool-shaped `{1,2,3}` rather than `0 | 6 = 1`); with `d{}` on either side and a pool on the other, AnyDice acts as identity on the pool (no-op preserving the pool's positional structure) rather than collapsing. Our `_apply_bool` `|`-branch implements the principled scalar-0-coercion path uniformly across these cases, so `d{} | <nonzero scalar>` yields `1` as expected; the seq-RHS and pool-other-side divergences are deliberate. Codified in `TestOr`'s anomaly probes and the `0x42aa4` / `0x1857f` annotations.
